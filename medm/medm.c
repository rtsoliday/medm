...
static menuEntry_t palettesMenu[] = {
  ...
  NULL,
  NULL,
  palettesMenuSimpleCallback,
  (XtPointer)PALETTES_CHANNEL_BTN,
  NULL},
#endif
                   {NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL},
  }
...
static void createMain() {
  ...
  n = 0;
  XtSetArg(args[n], XmNbackground, &defaultBackground);
  n++;
  XtSetArg(args[n], XmNforeground, &defaultForeground);
  n++;
  ...
  n = 0;
  XtSetArg(args[n], XmNmarginHeight, 9);
  n++;
  XtSetArg(args[n], XmNmarginWidth, 18);
  n++;
...
#  endif
  ...
  n = 0;
  XtSetArg(args[n], XtNiconName, "Help");
  n++;
  XtSetArg(args[n], XtNtitle, "Medm Help System");
  n++;
  ...
  n = 0;
  XtSetArg(args[n], XtNiconName, "EditHelp");
  n++;
  XtSetArg(args[n], XtNtitle, "Edit Help");
  n++;
  XtSetArg(args[n], XtNallowShellResize, TRUE);
  n++;
  XtSetArg(args[n], XmNkeyboardFocusPolicy, XmEXPLICIT);
...
void enableEditFunctions() {
  ...
  if (resourceS)
    XtSetSensitive(resourceS, True);
  if (colorS)
    XtSetSensitive(colorS, True);
  if (channelS)
    XtSetSensitive(channelS, True);
...
void disableEditFunctions() {
  ...
  if (resourceS)
    XtSetSensitive(resourceS, False);
  if (colorS)
    XtSetSensitive(colorS, False);
  if (channelS)
    XtSetSensitive(channelS, False);
  XtSetSensitive(mainEditPDM, False);
  XtSetSensitive(mainPalettesPDM, False);
}
