...
int dmGetBestFontWithInfo(XFontStruct **fontTable, int nFonts, char *text,
  ...
  *usedW = fontTable[i]->max_bounds.width;
  ...
  if (textWidthFlag) {
    ...
    while (((temp = XTextWidth(fontTable[i], text, strlen(text))) > w) && i > 0)
      i--;
...
DlElement *findSmallestTouchedExecuteElement(Widget w, DisplayInfo *displayInfo,
  ...
  while (w != displayInfo->drawingArea) {
    nargs = 0;
    XtSetArg(args[nargs], XmNx, &x0);
    nargs++;
    XtSetArg(args[nargs], XmNy, &y0);
    nargs++;
  ...
  while (TRUE) {
    ...
    case ButtonRelease:
      ...
      if (doSnap && snap) {
        ...
        w = ((w + gridSpacing / 2) / gridSpacing) * gridSpacing;
        h = ((h + gridSpacing / 2) / gridSpacing) * gridSpacing;
        if ((int)w < gridSpacing)
          w = gridSpacing;
        if ((int)h < gridSpacing)
          h = gridSpacing;
      }
      *initialX = x;
      *initialY = y;
  ...
  while (TRUE) {
    ...
    case MotionNotify:
      ...
      if (snap) {
        ...
        while (x < groupDeltaX0)
          x += gridSpacing;
        while (x > (int)(daWidth - groupDeltaX1))
          x -= gridSpacing;
        while (y < groupDeltaY0)
          y += gridSpacing;
        while (y > (int)(daHeight - groupDeltaY1))
          y -= gridSpacing;
      } else {
        /* Insure nothing goes outside the display */
...
void drawGrid(DisplayInfo *displayInfo) {
  ...
  if (!displayInfo || !displayInfo->drawingArea || !draw)
    return;
  ...
  while (dlElement != cdi->selectedDlElementList->head) {
    ...
    if (ele->type != DL_Display) {
      ...
      case ALIGN_HORIZ_CENTER:
        ...
        xOffset = (deltaX - (int)ele->structure.rectangle->object.width / 2) - ele->structure.rectangle->object.x;
        yOffset = 0;
        break;
      case ALIGN_HORIZ_RIGHT:
        /* Want   x + w = maxX  , therefore   x = maxX - w  */
        xOffset = (maxX - (int)ele->structure.rectangle->object.width) - ele->structure.rectangle->object.x;
        yOffset = 0;
        break;
      case ALIGN_VERT_TOP:
        xOffset = 0;
      ...
      case ALIGN_VERT_CENTER:
        ...
        xOffset = 0;
        yOffset = (deltaY - (int)ele->structure.rectangle->object.height / 2) - ele->structure.rectangle->object.y;
        break;
      case ALIGN_VERT_BOTTOM:
        /* Want   y + h = maxY  , therefore   y = maxY - h  */
        xOffset = 0;
        yOffset = (maxY - (int)ele->structure.rectangle->object.height) - ele->structure.rectangle->object.y;
        break;
      }
      if (ele->run->move) {
...
void alignSelectedElementsToGrid(Boolean edges) {
  ...
  while (dlElement != cdi->selectedDlElementList->head) {
    ...
    if (pE->type != DL_Display) {
      ...
      xoff = x - x0;
      if (xoff > gridSpacing / 2)
        xoff = xoff - gridSpacing;
      x00 = x - xoff;

      y = pE->structure.rectangle->object.y;
      y0 = (y / gridSpacing) * gridSpacing;
      yoff = y - y0;
      if (yoff > gridSpacing / 2)
        yoff = yoff - gridSpacing;
      y00 = y - yoff;
...
void freeNameValueTable(NameValueTable *nameValueTable, int numEntries) {
  ...
  if (nameValueTable != NULL) {
    for (i = 0; i < numEntries; i++) {
      if (nameValueTable[i].name != NULL)
        free((char *)nameValueTable[i].name);
      if (nameValueTable[i].value != NULL)
        free((char *)
             nameValueTable[i]
             .value);
    }
...
DisplayInfo *findDisplay(char *filename, char *argsString,
  ...
  /* Free the nameTable */
  if (nameTable)
    freeNameValueTable(nameTable, numNameValues);
    ...
    if (cmd[i] != '&') {
      *(command + ic) = *(cmd + i);
      ...
      case 'T':
        ...
        while (*name != '\0')
          if (*name++ == MEDM_DIR_DELIMITER_CHAR)
            title = name;
        len = strlen(title);
        if (ic + len >= 1024) {
          medmPostMsg(1, "parseAndExecCommand: Command is too long\n");
      ...
      case 'X':
        ...
        if (displayInfo->shell)
          u = (unsigned)XtWindow(displayInfo->shell);
        if (!u) {
...
char *shortName(char *filename) {
  ...
  if (filename) {
    ...
    while (*ptr != '\0')
      if (*ptr++ == MEDM_DIR_DELIMITER_CHAR)
        shortName = ptr;
  }
  return shortName;
}
