...
ArcRuntime::ArcRuntime(ArcElement *element)
  : QObject(element)
  , element_(element)
{
  ...
  if (element_->visibilityMode() == TextVisibilityMode::kCalc) {
    ...
    if (!calcExpr.isEmpty()) {
      ...
      if (status == 0) {
        ...
        calcValid_ = false;
        qWarning() << "Invalid visibility calc expression for arc element:"
...
void ArcRuntime::initializeChannels()
{
  if (!element_) {
    return;
  }
  ...
  for (auto &channel : channels_) {
    ...
    int status = ca_create_channel(channelBytes.constData(),
        &ArcRuntime::channelConnectionCallback, nullptr,
...
void ArcRuntime::cleanupChannels()
{
  for (auto &channel : channels_) {
...
void ArcRuntime::subscribeChannel(ChannelRuntime &channel)
{
  ...
  int status = ca_create_subscription(channel.subscriptionType, count,
      ...
      &ArcRuntime::valueEventCallback, &channel,
...
void ArcRuntime::unsubscribeChannel(ChannelRuntime &channel)
{
  if (channel.subscriptionId) {
    ca_clear_subscription(channel.subscriptionId);
    channel.subscriptionId = nullptr;
...
void ArcRuntime::requestControlInfo(ChannelRuntime &channel)
{
  ...
  int status = ca_array_get_callback(controlType, 1, channel.channelId,
      &ArcRuntime::controlInfoCallback, &channel);
  if (status != ECA_NORMAL) {
...
void ArcRuntime::handleChannelConnection(ChannelRuntime &channel,
...
void ArcRuntime::handleChannelValue(ChannelRuntime &channel,
...
void ArcRuntime::handleChannelControlInfo(ChannelRuntime &channel,
    const event_handler_args &args)
{
  if (!started_) {
    return;
...
bool ArcRuntime::evaluateCalcExpression(double &result) const
{
...
void ArcRuntime::channelConnectionCallback(connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
...
void ArcRuntime::valueEventCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
...
void ArcRuntime::controlInfoCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
