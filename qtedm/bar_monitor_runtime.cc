...
BarMonitorRuntime::BarMonitorRuntime(BarMonitorElement *element)
  : QObject(element)
  , element_(element)
{
  if (element_) {
...
BarMonitorRuntime::~BarMonitorRuntime()
{
  stop();
}

void BarMonitorRuntime::start()
{
  ...
  int status = ca_create_channel(channelBytes.constData(),
      &BarMonitorRuntime::channelConnectionCallback, this,
...
void BarMonitorRuntime::resetRuntimeState()
{
  connected_ = false;
  fieldType_ = -1;
  elementCount_ = 1;
...
void BarMonitorRuntime::subscribe()
{
  ...
  int status = ca_create_subscription(DBR_TIME_DOUBLE, 1, channelId_,
      DBE_VALUE | DBE_ALARM,
      &BarMonitorRuntime::valueEventCallback, this, &subscriptionId_);
  if (status != ECA_NORMAL) {
...
void BarMonitorRuntime::requestControlInfo()
{
  ...
  int status = ca_array_get_callback(DBR_CTRL_DOUBLE, 1, channelId_,
      &BarMonitorRuntime::controlInfoCallback, this);
  if (status == ECA_NORMAL) {
    ca_flush_io();
  } else {
...
void BarMonitorRuntime::handleConnectionEvent(const connection_handler_args &args)
{
  ...
  if (args.op == CA_OP_CONN_UP) {
    ...
    if (!isNumericFieldType(fieldType_)) {
      qWarning() << "Bar channel" << channelName_ << "is not numeric";
      invokeOnElement([](BarMonitorElement *element) {
        element->setRuntimeConnected(false);
        element->setRuntimeSeverity(kInvalidSeverity);
      });
...
void BarMonitorRuntime::handleValueEvent(const event_handler_args &args)
{
  ...
  if (severity != lastSeverity_) {
    ...
    invokeOnElement([severity](BarMonitorElement *element) {
  ...
  if (!hasLastValue_ || std::abs(numericValue - lastValue_) > 1e-12) {
    ...
    hasLastValue_ = true;
    invokeOnElement([numericValue](BarMonitorElement *element) {
      element->setRuntimeValue(numericValue);
    });
  }
}

void BarMonitorRuntime::handleControlInfo(const event_handler_args &args)
{
  if (!started_ || args.usr != this || !args.dbr) {
    return;
  }
  ...
  invokeOnElement([low, high, precision](BarMonitorElement *element) {
    element->setRuntimeLimits(low, high);
    element->setRuntimePrecision(precision);
  });
}

void BarMonitorRuntime::channelConnectionCallback(struct connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
  auto *self = static_cast<BarMonitorRuntime *>(ca_puser(args.chid));
  if (self) {
    self->handleConnectionEvent(args);
  }
}

void BarMonitorRuntime::valueEventCallback(struct event_handler_args args)
{
  if (!args.usr) {
    return;
  }
  auto *self = static_cast<BarMonitorRuntime *>(args.usr);
  if (self) {
    self->handleValueEvent(args);
  }
}

void BarMonitorRuntime::controlInfoCallback(struct event_handler_args args)
{
  if (!args.usr) {
    return;
  }
