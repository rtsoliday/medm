...
LineRuntime::LineRuntime(LineElement *element)
  : QObject(element)
  , element_(element)
{
  ...
  if (element_->visibilityMode() == TextVisibilityMode::kCalc) {
    ...
    if (!calcExpr.isEmpty()) {
      ...
      if (status == 0) {
        ...
        calcValid_ = false;
        qWarning() << "Invalid visibility calc expression for line element:"
...
void LineRuntime::initializeChannels()
{
  if (!element_) {
    return;
  }
  ...
  for (auto &channel : channels_) {
    ...
    int status = ca_create_channel(channelBytes.constData(),
        &LineRuntime::channelConnectionCallback, nullptr,
...
void LineRuntime::cleanupChannels()
{
  for (auto &channel : channels_) {
...
void LineRuntime::subscribeChannel(ChannelRuntime &channel)
{
  ...
  int status = ca_create_subscription(channel.subscriptionType, count,
      ...
      &LineRuntime::valueEventCallback, &channel,
...
void LineRuntime::unsubscribeChannel(ChannelRuntime &channel)
{
  if (channel.subscriptionId) {
    ca_clear_subscription(channel.subscriptionId);
    channel.subscriptionId = nullptr;
...
void LineRuntime::requestControlInfo(ChannelRuntime &channel)
{
  ...
  int status = ca_array_get_callback(controlType, 1, channel.channelId,
      &LineRuntime::controlInfoCallback, &channel);
  if (status != ECA_NORMAL) {
...
void LineRuntime::handleChannelConnection(ChannelRuntime &channel,
...
void LineRuntime::handleChannelValue(ChannelRuntime &channel,
...
void LineRuntime::handleChannelControlInfo(ChannelRuntime &channel,
    const event_handler_args &args)
{
  if (!started_) {
    return;
...
void LineRuntime::evaluateState()
{
  if (!element_) {
    return;
  }
...
bool LineRuntime::evaluateCalcExpression(double &result) const
{
...
void LineRuntime::channelConnectionCallback(connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
...
void LineRuntime::valueEventCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
...
void LineRuntime::controlInfoCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
