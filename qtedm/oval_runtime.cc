...
OvalRuntime::OvalRuntime(OvalElement *element)
  : QObject(element)
  , element_(element)
{
...
OvalRuntime::~OvalRuntime()
{
  stop();
}

void OvalRuntime::start()
{
  ...
  if (element_->visibilityMode() == TextVisibilityMode::kCalc) {
    ...
    if (!calcExpr.isEmpty()) {
      ...
      if (status == 0) {
        ...
        calcValid_ = false;
        qWarning() << "Invalid visibility calc expression for oval element:"
...
void OvalRuntime::initializeChannels()
{
  ...
  for (auto &channel : channels_) {
    ...
    int status = ca_create_channel(channelBytes.constData(),
        &OvalRuntime::channelConnectionCallback, nullptr,
...
void OvalRuntime::cleanupChannels()
{
  for (auto &channel : channels_) {
...
void OvalRuntime::subscribeChannel(ChannelRuntime &channel)
{
  ...
  int status = ca_create_subscription(channel.subscriptionType, count,
      ...
      &OvalRuntime::valueEventCallback, &channel,
...
void OvalRuntime::unsubscribeChannel(ChannelRuntime &channel)
{
  if (channel.subscriptionId) {
    ca_clear_subscription(channel.subscriptionId);
    channel.subscriptionId = nullptr;
...
void OvalRuntime::requestControlInfo(ChannelRuntime &channel)
{
  ...
  int status = ca_array_get_callback(controlType, 1, channel.channelId,
      &OvalRuntime::controlInfoCallback, &channel);
  if (status != ECA_NORMAL) {
...
void OvalRuntime::handleChannelConnection(ChannelRuntime &channel,
...
void OvalRuntime::handleChannelValue(ChannelRuntime &channel,
...
void OvalRuntime::handleChannelControlInfo(ChannelRuntime &channel,
    const event_handler_args &args)
{
  if (!started_) {
    return;
...
void OvalRuntime::evaluateState()
{
  if (!element_) {
    return;
  }
...
bool OvalRuntime::evaluateCalcExpression(double &result) const
{
...
void OvalRuntime::channelConnectionCallback(connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
...
void OvalRuntime::valueEventCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
...
void OvalRuntime::controlInfoCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
