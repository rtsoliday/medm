...
PolygonRuntime::PolygonRuntime(PolygonElement *element)
  : QObject(element)
  , element_(element)
{
...
PolygonRuntime::~PolygonRuntime()
{
  stop();
}

void PolygonRuntime::start()
{
  ...
  if (element_->visibilityMode() == TextVisibilityMode::kCalc) {
    ...
    if (!calcExpr.isEmpty()) {
      ...
      if (status == 0) {
        ...
        calcValid_ = false;
        qWarning() << "Invalid visibility calc expression for polygon element:"
...
void PolygonRuntime::resetState()
{
  calcPostfix_.clear();
  calcValid_ = false;
...
void PolygonRuntime::initializeChannels()
{
  if (!element_) {
    return;
  }
  ...
  for (auto &channel : channels_) {
    ...
    int status = ca_create_channel(channelBytes.constData(),
        &PolygonRuntime::channelConnectionCallback, nullptr,
...
void PolygonRuntime::cleanupChannels()
{
  for (auto &channel : channels_) {
...
void PolygonRuntime::subscribeChannel(ChannelRuntime &channel)
{
  ...
  int status = ca_create_subscription(channel.subscriptionType, count,
      ...
      &PolygonRuntime::valueEventCallback, &channel,
...
void PolygonRuntime::unsubscribeChannel(ChannelRuntime &channel)
{
  if (channel.subscriptionId) {
    ca_clear_subscription(channel.subscriptionId);
    channel.subscriptionId = nullptr;
...
void PolygonRuntime::requestControlInfo(ChannelRuntime &channel)
{
  ...
  int status = ca_array_get_callback(controlType, 1, channel.channelId,
      &PolygonRuntime::controlInfoCallback, &channel);
  if (status != ECA_NORMAL) {
...
void PolygonRuntime::handleChannelConnection(ChannelRuntime &channel,
...
void PolygonRuntime::handleChannelValue(ChannelRuntime &channel,
...
void PolygonRuntime::handleChannelControlInfo(ChannelRuntime &channel,
    const event_handler_args &args)
{
  if (!started_) {
    return;
  ...
  case DBR_CTRL_DOUBLE: {
    const auto *value = static_cast<const dbr_ctrl_double *>(args.dbr);
    channel.hopr = value->upper_disp_limit;
    channel.lopr = value->lower_disp_limit;
    channel.precision = value->precision;
...
void PolygonRuntime::evaluateState()
{
  if (!element_) {
    return;
  }
...
bool PolygonRuntime::evaluateCalcExpression(double &result) const
{
...
void PolygonRuntime::channelConnectionCallback(connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
...
void PolygonRuntime::valueEventCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
...
void PolygonRuntime::controlInfoCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
