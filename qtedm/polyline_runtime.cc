...
PolylineRuntime::~PolylineRuntime()
{
  stop();
}

void PolylineRuntime::start()
{
  ...
  if (element_->visibilityMode() == TextVisibilityMode::kCalc) {
    ...
    if (!calcExpr.isEmpty()) {
      ...
      if (status == 0) {
        ...
        calcValid_ = false;
        qWarning() << "Invalid visibility calc expression for polyline element:"
...
void PolylineRuntime::resetState()
{
  calcPostfix_.clear();
  calcValid_ = false;
...
void PolylineRuntime::initializeChannels()
{
  if (!element_) {
    return;
  }
  ...
  for (auto &channel : channels_) {
    ...
    int status = ca_create_channel(channelBytes.constData(),
        &PolylineRuntime::channelConnectionCallback, nullptr,
...
void PolylineRuntime::cleanupChannels()
{
  for (auto &channel : channels_) {
...
void PolylineRuntime::subscribeChannel(ChannelRuntime &channel)
{
  ...
  int status = ca_create_subscription(channel.subscriptionType, count,
      ...
      &PolylineRuntime::valueEventCallback, &channel,
...
void PolylineRuntime::unsubscribeChannel(ChannelRuntime &channel)
{
  if (channel.subscriptionId) {
    ca_clear_subscription(channel.subscriptionId);
    channel.subscriptionId = nullptr;
...
void PolylineRuntime::requestControlInfo(ChannelRuntime &channel)
{
  ...
  int status = ca_array_get_callback(controlType, 1, channel.channelId,
      &PolylineRuntime::controlInfoCallback, &channel);
  if (status != ECA_NORMAL) {
...
void PolylineRuntime::handleChannelConnection(ChannelRuntime &channel,
...
void PolylineRuntime::handleChannelValue(ChannelRuntime &channel,
...
void PolylineRuntime::handleChannelControlInfo(ChannelRuntime &channel,
    const event_handler_args &args)
{
  if (!started_) {
    return;
...
void PolylineRuntime::evaluateState()
{
  if (!element_) {
    return;
  }
...
bool PolylineRuntime::evaluateCalcExpression(double &result) const
{
...
void PolylineRuntime::channelConnectionCallback(connection_handler_args args)
{
  if (!args.chid) {
    return;
  }
...
void PolylineRuntime::valueEventCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
...
void PolylineRuntime::controlInfoCallback(event_handler_args args)
{
  if (!args.usr) {
    return;
  }
