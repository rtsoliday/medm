...
void printUsage(const QString &program)
{
  ...
  fprintf(stdout,
      ...
      "Options:\n"
      "  --read-only\n"
      "            Start in execute mode and block CA, PVA, soft-PV, and\n"
      "            plugin-provider writes. Monitoring, navigation, and\n"
      ...
      "\n"
      "  EPICS_DISPLAY_PATH=directory-list\n"
      "            Search path for ADL files and referenced images. Uses the\n"
      "            platform path-list separator (: on Unix, ; on Windows).\n"
      "\n"
      "  QTEDM_AUDIT_DIR=directory\n"
      "            Audit-log directory. The default is ~/.medm.\n"
      "\n"
      "  QTEDM_ARCHIVER_PROVIDER=provider-id\n"
      "            Archive-provider plugin ID. Empty or archiver-appliance\n"
      "            selects the built-in provider.\n"
      "\n"
      "  QTEDM_PLUGIN_PATH=directory-list\n"
      "            Absolute local plugin directories separated with the\n"
      "            platform path-list separator.\n"
      "\n"
      "  MEDM_EXEC_LIST=entries\n"
      "            MEDM-compatible semicolon-delimited commands added to the\n"
      "            execute-mode context menu.\n"
      "\n"
      "  QTEDM_NOLOG=1\n"
      "            Disable control-write audit logging.\n"
      "\n"
