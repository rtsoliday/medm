...
def verify(base="/"):
    pages = {}
    # Leave historical MEDM links alone, but validate the current standalone
    # QtEDM manual and showcase along with the modern pages.
    for path in OUTPUT.rglob('*.html'):
        if path == OUTPUT / 'manual/MEDM.html':
            continue
        parser = Page()
    ...
    for path, page in pages.items():
        ...
        for tag, url, attrs in page.links:
            ...
            if (tag == 'a' and dest.is_file() and dest.suffix == '.html'
                    and 'manual' in dest.relative_to(OUTPUT).parts
                    and 'manual' not in path.relative_to(OUTPUT).parts
    ...
    # Every named help anchor must survive the conversion, including targets
    # not currently referenced by another page.
    manual = Page()
    manual.feed((ROOT / 'docs/QtEDM.html').read_text())
    web_ids = set().union(*(p.ids for path, p in pages.items()
                           if 'manual' not in path.relative_to(OUTPUT).parts))
    for ident in manual.ids - {'Contents'} - web_ids:
        issues.append(f'Manual anchor lost during conversion: {ident}')
    # Check actual prose and examples, not only navigation: a renderer change
    # must not silently omit a paragraph, list item, or code example.
    source = runpy.run_path(str(ROOT / 'scripts/sync-docs.py'))
    tree = source['Manual']()
    tree.feed((ROOT / 'docs/QtEDM.html').read_text())
    body = next(n for n in tree.root.walk() if n.tag == 'body')
    section, passages = None, 0
    normalize = lambda text: re.sub(r'\s+', '', text)
    for node in body.children:
        if isinstance(node, str):
            continue
        if node.tag == 'h2':
            section = source['anchor'](node)
        if section not in source['SECTIONS']:
            continue
        route = source['SECTIONS'][section][0]
        page = pages[(OUTPUT / (route + '.html')).resolve()]
        rendered = normalize(''.join(page.text))
        for part in node.walk():
            if part.tag in ('p', 'li', 'pre'):
                passages += 1
                if normalize(part.text()) not in rendered:
                    issues.append(f'{route}: missing manual text: {part.text()[:80]}')
    if issues:
        raise RuntimeError('\n'.join(sorted(set(issues))))
    print(f'Validated {len(pages)} HTML pages, {links} local links/assets, '
          f'{len(manual.ids) - 1} manual anchors, and {passages} source passages.')
...
def main():
    ...
    args.add_argument('--base', default=os.environ.get('DOCS_BASE', '/'),
                      help='Hosting URL prefix, e.g. /manuals/QtEDM/ (default: /)')
